...
class GoCosiRPCServicer(gocosi_pb2_grpc.GocosiRPCServicer):
    def __init__(self, _host, _port, _sum_nodes, _logger):
        ...
        private_key, public_key = BLS.createKey()
        self.private_key = private_key
        self.public_key = public_key
        self.public_key_dict[self.address] = str(bytes(self.public_key), "ISO-8859-1")
        ...
        try:
            if len(self.sub_list) <= 0:
                ...
                for i in range(len(signers)):
                    if signers[i] > 0:
                        node_port = self.port - self.port % self.sum_nodes + i
                        node = self.host + ":" + str(node_port)
                        co_signers[node] = signers[i]
                return gocosi_pb2.NewMsgResp(signature=co_sig, signers=co_signers)
        except Exception as e:
            traceback.print_exc()
            return gocosi_pb2.CommonResp(message=str(e))
    ...
    def sig_for_msg(self, values):
        ...
        if message not in self.msg_list:
            if self.is_valid_msg(message, co_sig, signers):
                ...
                co_sig_str = str(bytes(co_sig), "ISO-8859-1")
                signers[self.port % self.sum_nodes] = 1
                sigs = {
                    'co_sig': co_sig_str,
                    'signers': signers
                }
            ...
            if update:
                ...
                new_co_sig_str = str(bytes(new_co_sig), "ISO-8859-1")
                sigs = {
    ...
    def new_msg(self, message):
        ...
        msg = bytes(temp_array)
        sig = BasicSchemeMPL.sign(self.private_key, msg)
        sig_str = str(bytes(sig), "ISO-8859-1")
    ...
    def get_msg_cosig(self, msg):
        ...
        while time.time() - start < self.msg_time_out:
            ...
            if signers_length / self.sum_nodes > 0.66:
                # print(len(sigs))
                # print(len(self.sub_list))
                # self.msg_sig_dict.pop(msg)
